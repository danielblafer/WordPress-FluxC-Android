...
import org.wordpress.android.fluxc.model.list.ListManager.SectionedListManager
...
class ListStore @Inject constructor(
    ...
    suspend fun <T> getSectionedListManager(
        listDescriptorList: List<ListDescriptor>,
        dataSource: ListItemDataSource<T>,
        loadMoreOffset: Int = DEFAULT_LOAD_MORE_OFFSET
    ): SectionedListManager<T> = withContext(Dispatchers.Default) {
        val listManagers = listDescriptorList.map { getListManager(it, dataSource, loadMoreOffset) }
        return@withContext SectionedListManager(listManagers)
    }
